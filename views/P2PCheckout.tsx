...
export const P2PCheckout: React.FC<P2PCheckoutProps> = ({ items, onFinish }) => {
  ...
  const [messages, setMessages] = useState([
    { id: 1, sender: 'system', text: 'Orden #9902 Creada. Esperando confirmación del vendedor.' },
    { id: 2, sender: 'seller', text: '¡Hola! Gracias por tu pedido. Acepto Pago Móvil y Efectivo.' },
  ]);
  ...
  const handlePaymentProof = () => {
    setShowUpload(false);
    setMessages([...messages, { id: Date.now(), sender: 'system', text: '📸 Comprobante de pago enviado. Esperando validación.' }]);
    setStatus('paid');
    ...
    <div className="flex flex-col h-full bg-slate-100 dark:bg-slate-900 animate-fade-in-up">
       ...
       <header className="bg-slate-800 text-white p-4 flex items-center justify-between shadow-md">
           <div className="flex items-center gap-3">
               ...
               <div>
                   ...
                   <p className="text-xs text-slate-300">Orden #9902 • {items[0].seller}</p>
               </div>
           </div>
       ...
       <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
           ...
           <div className="bg-white dark:bg-slate-800 p-4 border-b md:border-b-0 md:border-r border-gray-200 dark:border-slate-700 md:w-80 overflow-y-auto">
               ...
               <div className="mt-6 bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg border border-yellow-100 dark:border-yellow-900/30">
                       ...
                       <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                           {msg.sender === 'system' ? (
                               <div className="w-full flex justify-center my-2">
                                   <span className="bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wide">
                                       {msg.text}
                                   </span>
                               </div>
                           ) : (
               ...
               <div className="bg-white dark:bg-slate-800 p-3 border-t border-gray-200 dark:border-slate-700 pb-20 md:pb-3">
                   {status === 'pending' && (
                       <div className="flex gap-2 mb-3 overflow-x-auto pb-1">
                           <button 
                                ...
                                className="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 px-3 py-2 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300 whitespace-nowrap"
                            >
                               <span className="material-symbols-outlined text-base">receipt_long</span>
                               Notificar Pago
                           </button>
                           <button className="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 px-3 py-2 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300 whitespace-nowrap">
                   ...
                   {showUpload && (
                        <div className="absolute bottom-20 left-4 right-4 bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-4 border border-gray-200 dark:border-slate-600 animate-fade-in-up">
                            <h4 className="font-bold text-slate-800 dark:text-white mb-2">Reportar Pago</h4>
                            ...
                            <button onClick={handlePaymentProof} className="w-full bg-brand-green text-white py-2 rounded-lg font-bold">Enviar Comprobante</button>
                        </div>
                   )}
                   ...
                   <div className="flex gap-2">
                       ...
                       />
                       <button onClick={handleSend} className="bg-brand-blue text-white p-2 rounded-full shadow-lg">
